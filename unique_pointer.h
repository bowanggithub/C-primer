...
{
    friend void swap<T,D>(unique_pointer<T,D>& lhs,
                            unique_pointer<T,D>& rhs);
public:
    ...
    uniquepointer& operator =(unique_pointer&& rhs) noexcept;
...
inline void swap(unique_poiner<T,D>& lhs, nique_pointer<T,D>& rhs)
{
    using std::swap;
